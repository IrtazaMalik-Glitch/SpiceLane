...
/* ════════════════════════════════
   DATA — MENU ITEMS (with real Unsplash images)
════════════════════════════════ */
const menuItems = [
  // DESI
  ...
  // SNACKS
  { id:18, name:'Loaded Fries',        tag:'snack',  img:'https://images.unsplash.com/photo-1585325701165-c9aac31f4c78?w=600&q=80', desc:'Crispy fries topped with cheese sauce, jalapeños & chicken tikka.', price:220, rating:'4.5', badge:'new',
    options:{ type:'size', label:'Choose Size', choices:['Regular – Rs.220','Large – Rs.340'], prices:[220,340] }
  },
  { id:19, name:'Samosa Chaat',        tag:'snack',  img:'https://images.unsplash.com/photo-1606491956689-2ea866880c84?w=600&q=80', desc:'Crispy samosas with yoghurt, tamarind chutney & chaat masala.', price:150, rating:'4.8',
    options:{ type:'size', label:'Choose Quantity', choices:['2 Samosas – Rs.150','4 Samosas – Rs.270'], prices:[150,270] }
  },
  { id:20, name:'Pakora Platter',      tag:'snack',  img:'https://images.unsplash.com/photo-1603133872878-684f208fb84b?w=600&q=80', desc:'Mixed vegetable & chicken pakoras with mint chutney dip.', price:160, rating:'4.6',
...
const coffeeItems = [
  ...
  { id:103, name:'Caramel Latte',   emoji:'🥛', img:'https://images.unsplash.com/photo-1561882468-9110d70d0283?w=500&q=80', desc:'Smooth latte swirled with rich house-made caramel sauce.', price:220, sizes:['Medium','Large'] },
  ...
  { id:106, name:'Dalgona Coffee',  emoji:'☕', img:'https://images.unsplash.com/photo-1586195831895-5d5c5e0c1b3f?w=500&q=80', desc:'Whipped coffee cloud on creamy cold milk. The TikTok legend.', price:230, sizes:['Regular'], badge:'new' },
  { id:107, name:'Masala Chai',     emoji:'🫖', img:'https://images.unsplash.com/photo-1563729784474-d77dbb933a9e?w=500&q=80', desc:'Spiced desi tea with cardamom, ginger, cinnamon & cloves.', price:80, sizes:['Cup','Kettle'] },
  { id:108, name:'Kashmiri Pink Tea',emoji:'🌸',img:'https://images.unsplash.com/photo-1544025162-d76694265947?w=500&q=80', desc:'Traditional noon chai — creamy, salty & utterly comforting.', price:100, sizes:['Cup','Large'] },
];
...
function handleSearch(q) {
  ...
  searchTimeout = setTimeout(() => {
    ...
    const all = [
      ...
      ...drinksItems.map(d => ({ ...d, tag: 'drinks', emoji: '🥤', price: d.price }))
    ];
    ...
    resultsEl.innerHTML = hits.map(item => `
      <div class="search-result-item" onmousedown="addFromSearch(${JSON.stringify(item).replace(/"/g, '&quot;')})">
        <div class="search-result-emoji">${item.emoji || '🍽️'}</div>
...
function updateCustomPrice() {
  ...
  if (!priceEl) return;
  const price = selectedOption ? selectedOption.price : currentCustomItem.price;
...
function addCustomToCart() {
  ...
  if (hasOptions && !selectedOption) {
    showToast('Please choose a ' + item.options.label.toLowerCase() + ' ⚠️', 'error');
    return;
  }
  if (hasFlavours && !selectedFlavour) {
    showToast('Please choose a flavour ⚠️', 'error');
    return;
  }
...
function filterMenu(tag, btn) {
  if (btn) {
    ...
    document.querySelectorAll('.filter-btn').forEach(b => {
      b.classList.toggle('active', b.textContent.toLowerCase().includes(tag) || tag === 'all');
    });
  }
  ...
  document.getElementById('coffeeGrid').innerHTML = coffeeItems.map(c => `
    <div class="coffee-card reveal">
      <div class="coffee-img-wrap">
        ...
        <div class="coffee-emoji-overlay">${c.emoji}</div>
      </div>
...
function openCoffeeModal(c) {
  currentCustomItem = { ...c, options: { label: 'Choose Size', choices: c.sizes, prices: c.sizes.map((s, i) => c.price + i * 40) } };
  selectedOption = null;
  selectedFlavour = null;
  const modal = document.getElementById('customModal');
  const content = document.getElementById('customModalContent');
  ...
  // Override currentCustomItem to use flat structure
  currentCustomItem = { ...c, id: c.id, img: c.img, emoji: c.emoji, tag: 'coffee',
    options: { label: 'Choose Size', choices: c.sizes.map((s,i)=>`${s} — Rs.${c.price+i*40}`), prices: c.sizes.map((s,i)=>c.price+i*40) }
  };
  modal.classList.add('active');
}
...
function openDrinkModal(d) {
  currentCustomItem = {
    id: d.id, name: d.name, img: d.img, emoji: '🥤',
    price: d.price, tag: 'drinks',
    flavours: d.flavours
  };
  selectedOption = null; selectedFlavour = null;
  const modal = document.getElementById('customModal');
  const content = document.getElementById('customModalContent');
...
function addDrinkToCart(id, price) {
  ...
  addToCart({
    id: id + '_' + selectedFlavour,
    name: d.name + ' (' + selectedFlavour + ')',
    img: d.img, emoji: '🥤',
    price, customLabel: selectedFlavour, tag: 'drinks'
  });
  closeCustomModal();
}
...
function showOrder() {
  ...
  document.getElementById('modalOrderSummary').innerHTML = `
    ...
    ${cart.map(c => `<div class="osb-item"><span>${c.emoji || '🍽️'} ${c.name}${c.customLabel ? ' (' + c.customLabel + ')' : ''} ×${c.qty}</span><span>Rs.${c.price * c.qty}</span></div>`).join('')}
...
function handlePaymentChange(val) {
  ...
  bankDiv.style.display = (val === 'bank' || val === 'card') ? 'block' : 'none';
}
...
async function confirmOrder() {
  ...
  const fee = (orderType === 'pickup' || orderType === 'dine') ? 0 : (sub >= 800 ? 0 : 100);
  const total = sub + fee;
  const itemsList = cart.map(c => `${c.emoji || ''} ${c.name}${c.customLabel ? ' (' + c.customLabel + ')' : ''} ×${c.qty} — Rs.${c.price * c.qty}`).join('\n');
  ...
  // EmailJS (optional — fill your keys)
  try {
...
function animateStats() {
  const statObserver = new IntersectionObserver(entries => {
    entries.forEach(e => {
      ...
      // Special case for rating (49 → "4.9")
      if (target === 49) {
        let start = 40;
        const step = () => {
          start++;
